...
void ClientConnection::SaveConnectionHistory()
{
	...
	HKEY hKey;
	LONG result = registry->CreateKeyEx(HKEY_CURRENT_USER, KEY_VNCVIEWER_HISTORI,
	...
	for (i = 0; i < maxEntries; i++) {
		...
		LONG err = registry->QueryValueEx(hKey, valueName, 0, 0, bufPtr, &bufSize);
	...
	registry->SetValueEx(hKey, _T("0"), 0, REG_SZ, newConnPtr, newConnSize);
	...
	for (i = 0; i < numRead && numWritten < maxEntries; i++) {
		if (_tcscmp(connList[i], m_opts.m_display) != 0) {
			...
			registry->SetValueEx(hKey, keyName, 0, REG_SZ, bufPtr, bufSize);
			numWritten++;
		}
	}
	...
	if (i < numRead) {
		if (_tcscmp(connList[i], m_opts.m_display) != 0) {
			registry->DeleteKey(hKey, connList[i]);
		}
	}
