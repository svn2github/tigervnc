...
public:
  _RectRegion(const Rect& r) {
    ...
    region.numRects = 1;
    region.extents.x1 = r.left;
    region.extents.y1 = r.top;
    region.extents.x2 = r.right;
    region.extents.y2 = r.bottom;
    region.size = 1;
    if (r.isEmpty())
      region.numRects = 0;
  }
...
void rfb::Region::reset(const Rect& r) {
  if (r.isEmpty()) {
    ...
    xrgn->numRects = 1;
    xrgn->rects[0].x1 = xrgn->extents.x1 = r.left;
    xrgn->rects[0].y1 = xrgn->extents.y1 = r.top;
    xrgn->rects[0].x2 = xrgn->extents.x2 = r.right;
    xrgn->rects[0].y2 = xrgn->extents.y2 = r.bottom;
  }
}
...
bool rfb::Region::equals(const rfb::Region& r) const {
  return XEqualRegion(xrgn, r.xrgn) != 0;
}
    ...
    while (nRectsInBand > 0) {
      ...
      do {
        if (h > xrgn->rects[i].y2 - y)
          h = xrgn->rects[i].y2 - y;
        Rect r;
        r.setRect(xrgn->rects[i].x1, y, xrgn->rects[i].x2, y+h);
        rects->push_back(r);
        y += h;
...
Rect rfb::Region::get_bounding_rect() const {
  Rect r;
  r.setRect(xrgn->extents.x1, xrgn->extents.y1,
